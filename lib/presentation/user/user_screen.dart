...
class _UserScreenState extends State<UserScreen> {
  ...
  Future changePassword({String password}) async {
    if (_controllerPassword.text.length <= 5) {
      ...
      if (_controllerPassword.text == _controllerPasswordConfirm.text) {
        ...
        await user.reauthenticateWithCredential(authCredential).then((result) {
      ...
      Padding(
        ...
        child: SizedBox(
          ...
          child: MaterialButton(
            ...
            onPressed: () {
              changePassword(password: _controllerPassword.text);
            },
          ),
        ),
      ),
  ...
  void _validateAndSubmit(UserScreenViewModel vm) {
    if (_formKey.currentState.validate()) {
      ...
      vm.submit(
          vm.user.rebuild((u) => u..name = _controllerName.text), completer);
    }
  }
}
